...
console.log(`  line in force after this refit: ${linesServed.long} innings an appearance (${linesServed.source === 'save' ? "the league's own" : 'the starting value'}); measured ${sb?.measured?.toFixed(3) ?? '—'} on ${sb?.relievers ?? 0} relievers (${sb?.basis ?? 'no record'}; this export's attempt: ${sb?.attempt.reason ?? '—'}); checks: ${(std?.record.heldOut ?? []).filter((c) => c.part === 'long_line').map((c) => `${c.kind} ${c.observed === null ? 'not measured' : `${(c.observed * 100).toFixed(1)}%`} (aim ${((c.expected ?? 0) * 100).toFixed(0)}%) ${c.passed === null ? '' : c.passed ? 'pass' : 'FAIL'}`).join('; ')}`);
