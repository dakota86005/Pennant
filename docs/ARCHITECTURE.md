...
| Scouted evidence | `scoutedEvidence.ts` is the only reader of ability ratings for development and operations judgments. It returns branded `ScoutedAbility` evidence with provenance, viewer, scale, and what is missing. Since Player Value phase 6d the Roster's scouting column (`api.ts`: the scouts' tools averaged now and at their ceiling, the card header's "Scouted" figure) and the Lineup (`lineup.ts`: bats through the tools model on the split grades, gloves through `scoutedGloves`) read it in place of `players_value`. | Nothing in Player Development or Minor League Operations may read a rating column or `players_value` directly; `tests/evidenceBoundary.test.ts` enforces it. |
...
| MLB Operations | `mlbRoster.ts` (club view over Player State), `mlbNeeds.ts` (state-derived needs), `mlbResponses.ts` (staged candidates, transaction path, consequences, philosophy annotation), `mlbEvidence.ts` (adapters to the specialists), `mlbOperations.ts` (service + `/api/mlb-operations`), `MlbOperations.tsx`; the roster review's per-save yardsticks (D-053, cycle 1): `mlbCalibrationFit.ts` (pure method and policy), `mlbCalibrationRefit.ts` (objective reads and the refit registrations, the worker's), `mlbCalibration.ts` (the fit in force, the page line and its hover). | Consumer only (D-024, [MLB_OPERATIONS.md](MLB_OPERATIONS.md)); coverage numbers are floors, not targets: decides no scouting, development, rights, philosophy or farm assignment question. Reads no raw rating, roster-status, option or log source; `tests/mlbOperationsBoundary.test.ts` enforces it. |
...
A page that shows Player Value's figures says how current they are (Player Value A-20,
phase 6a): `freshnessCue` (`dataStatus.ts`) gives the export's game date, its state
against the save and one short line; Contracts, the player card's route and the
one-player value routes hand that state to Player Value as `currentState`, so a
stale export leaves service, control and what rests on them not established on the
page and the card alike, and Player Rights' unverified limitation reaches the GM.
...
season's service clock, `seasonServiceClocks`, capped by what the schedule has
left, `seasonServiceCalendars` reading the export's `games` dates) on the high
edge; on the low edge nothing, except that a player on the major-league injured
list banks the days left on his stint (the list accrues service, and he cannot be
optioned from it). Each later season adds what the league's schedule banks (its
calendar span, never more than the service year) on both edges; where the export
carries no schedule, a full service year, said as the provisional convention. A
threshold inside the band makes that season `indeterminate` and names the season
on each side. Arbitration trips count winters, not service classes: this season's
is a floor from the service class (an earlier Super Two year is not exported), and
each later one follows the one before. The year before the arbitration line is
decided by Super Two where the league's regime as read is MLB's and its schedule
banks a full service year (owner ruling, 2026-09-22; basis `owner_attested`):
`superTwoCutoffs` ranks the class once per request or league pass
...
Player Value's lens (`playerValueLens.ts`, D-052, PLAYER_VALUE.md 6.1) is the same boundary for value: the neutral
valuation is computed without philosophy, and the lens runs afterwards, at read time, on the valuation as served:

```text
neutral valuation (no philosophy; every read serves the same one)
        |
        v  settings -> philosophyForOrg(viewing organization) -> resolvePhilosophy   (ourViewRoutes.ts, read time)
ourViewOf(neutral, philosophy) -> "our view" beside neutral: each lean named, by how much; the neutral figures unchanged
```

It re-weights seasons, reads a range more cautiously and weighs cost, control and guaranteed money; it never changes a
neutral figure, turns an unknown into a number, or reads the club's value of a win (a club fact from the standings,
served beside it). A philosophy edit recomputes nothing.
...
Calibration belongs to the save. Three subsystem-neutral modules carry every subsystem's per-save fits except Player Value's
(which predates them and keeps `value_production_fits`, not migrated):

- `saveIdentity.ts`: the save's identity (its configured name and a fingerprint of the league's own history), the league's
  seasons and schedules, and the last completed season, moved unchanged from Player Value (byte-identical fingerprint,
  `tests/saveIdentity.test.ts`); Player Value re-exports them.
- `saveCalibrationStore.ts`: `history.db` `save_calibration_fits`, keyed by save identity, league, subsystem, component, method
  and basis (a completed season, or an export's game date for a measurement of the league as it stands); additive and
  idempotent; a failed fit is recorded with its reason and never replaces an adopted one; a fit through a season the league has
