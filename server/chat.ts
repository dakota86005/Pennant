...
export function systemPrompt(orgId: number, persona: Persona): string {
  ...
  return [
    ...
    'means he cannot leave at all, and "indeterminate" means the save cannot yet establish which',
    '(its reason says why) — say that, and never pick one. "option" means next season is an option or his',
    'opt-out: exercised he is under contract at the salary, declined (or opted out) he falls to its "ifDeclined";',
    'give both branches and never call him signed for that season. Never call a man a free agent, a rental, or in his last year',
