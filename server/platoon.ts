...
import { blendStabilization, reliability, RESULTS_PRIOR, wobaOf, type BattingLine } from './resultsMetrics.js';
...
export interface PlatoonInput {
  ...
  /**
   * The sample at which his overall record counts as much as his ratings, under the results params in force (D-053; `mlbEvidence`
   * always sets it). Absent only in a hand-built case, where the starting value serves.
   */
...
export function evaluatePlatoon(input: PlatoonInput): PlatoonRead {
  ...
  const wRecord = recordLevel !== null ? reliability(all.pa, input.recordStabilization ?? blendStabilization('hitter', RESULTS_PRIOR)) : 0;
